...
自原作者 zhengxinlan1995-code 的 2.6.1 版本以来到 3.8.4 版本（截至 2026-02-04）共经历 至少 141 次 快速迭代与修复（含 85 个公开版本号及 56+ 次内部逻辑微测），包含 6 个 里程碑版本与 3 次 重构。目前已进化为功能较为完善的工作流设计生产平台。
...
### 🚀 最新稳定版 (Latest Sync)
*   **版本号**: **v3.8.4** (2026-02-04)
*   **主要更新**: 
    *    **本地 ComfyUI 接入**：通用 asyncConfig 轮询与 outputs 解析打通。
    *    **输出兼容**：支持 `object_url`/`outputs` 多形态解析。
    *    **参数上限提升**：自定义参数值上限提升至 50。
    *    **模型库完善**：请求模板预览/覆盖、模型复制与导入导出流程更稳。
