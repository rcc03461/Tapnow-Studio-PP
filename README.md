...
自原作者 [@zhengxinlan1995-code](https://github.com/zhengxinlan1995-code) 的 2.6.1 版本以来到 3.8.3-rc7 版本（截至 2026-02-02）共经历 至少 141 次 快速迭代与修复（含 85 个公开版本号及 56+ 次内部逻辑微测），包含 6 个 里程碑版本与 3 次 重构。目前已进化为功能较为完善的工作流设计生产平台。
...
### 🚀 最新稳定版 (Latest Sync)
*   **文件名**: `Tapnow Studio-V3.8.2.html`
*   **版本号**: **v3.8.2** (2026-01-28)
*   **主要更新**: 
    *    **缩略图重建修复**：统一走资源获取通道，避免 CORS 污染。
    *    **资源获取统一**：本地缓存/代理/直连统一入口，下载/引用一致。
    *    **控制台优化**：移除自动缓存高频调试日志。

### 🧪 最近开发版 (进行中)
*   **文件名**: `Tapnow Studio-V3.8.3-rc7.html`
*   **版本号**: **v3.8.3-rc7** (2026-02-02)
*   **主要更新**:
    *    **模型库请求模板**：Jimeng 视频模板默认启用，支持首尾帧 files 与 auto bodyType。
    *    **模板变量扩展**：支持 first/last frame 与 jimengRatio/jimengDuration/jimengResolution。
    *    **黑屏防护**：启动护栏 + ErrorBoundary + 全局错误捕获。
